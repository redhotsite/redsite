...
def log_action(level, action, details=None, user=None):
    """Função auxiliar para registrar logs no sistema"""
    log = Log(
        level=level,
        action=action,
        details=details,
...
class User(db.Model):
    ...
    destaques_recebidos = db.relationship('Destaque', backref='user', foreign_keys='Destaque.user_id', lazy=True)
    destaques_dados = db.relationship('Destaque', backref='created_by_user', foreign_keys='Destaque.created_by', lazy=True)
    noticias = db.relationship('Noticia', backref='author', lazy=True)
    area_posts = db.relationship('AreaPost', backref='author', lazy=True)
    pending_approvals = db.relationship('PendingUser', backref=db.backref('approver', lazy=True), lazy=True)
    gemas_criadas = db.relationship('Gema', backref='created_by_user', foreign_keys='Gema.created_by', lazy=True)
    cargos_confianca = db.relationship('CargoConfianca', secondary=user_cargos_confianca, backref='users')
    total_gemas = db.Column(db.Integer, default=0)
    is_active = db.Column(db.Boolean, default=True)
    
    def atualizar_cargo_por_gemas(self):
        # Busca todos os cargos por gemas ordenados por min_gemas
        cargos = CargoGema.query.order_by(CargoGema.min_gemas).all()
        
        # Encontra o cargo adequado baseado no total de gemas
        cargo_adequado = None
        for cargo in cargos:
            if self.total_gemas >= cargo.min_gemas and self.total_gemas <= cargo.max_gemas:
                cargo_adequado = cargo
                break
        
        # Atualiza o cargo do usuário
        if cargo_adequado:
            self.cargo_gema_id = cargo_adequado.id
            log_action('info', 'Cargo por gemas atualizado', 
                      f'Usuário {self.username} teve seu cargo por gemas atualizado para {cargo_adequado.nome}')
        else:
            # Se não encontrar cargo adequado, remove o cargo atual
            if self.cargo_gema_id:
                self.cargo_gema_id = None
                log_action('info', 'Cargo por gemas removido', 
                          f'Usuário {self.username} teve seu cargo por gemas removido por não atingir os requisitos')
        
        db.session.commit()
        return cargo_adequado
...
def init_db():
    with app.app_context():
        ...
        log_action('info', 'Sistema iniciado', 'Banco de dados verificado e inicializado')
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            ...
            log_action('info', 'Login', f'Usuário {username} fez login no sistema', user)
            flash('Login realizado com sucesso!', 'success')
            return redirect(url_for('dashboard'))
        
        log_action('warning', 'Tentativa de login falha', f'Tentativa de login mal sucedida para o usuário {username}')
...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            log_action('warning', 'Registro falhou', f'Tentativa de registro com username já existente: {username}')
            flash('Nome de usuário já está em uso.', 'error')
            return redirect(url_for('register'))

        # Verificar se email já existe
        if User.query.filter_by(email=email).first():
            log_action('warning', 'Registro falhou', f'Tentativa de registro com email já existente: {email}')
            flash('E-mail já está em uso.', 'error')
            return redirect(url_for('register'))

        # Verificar se discord já existe
        if User.query.filter_by(discord=discord).first():
            log_action('warning', 'Registro falhou', f'Tentativa de registro com discord já existente: {discord}')
        ...
        try:
            ...
            log_action('info', 'Novo usuário registrado', f'Usuário {username} criou uma conta', new_user)
            flash('Conta criada com sucesso! Faça login para continuar.', 'success')
            return redirect(url_for('login'))
        except Exception as e:
            db.session.rollback()
            log_action('error', 'Erro no registro', f'Erro ao criar conta para {username}: {str(e)}')
...
def dashboard():
    ...
    try:
        ...
        # Busca o cargo por gemas atual
        cargo_gemas = None
        if user.cargo_gema_id:
            cargo_gema = CargoGema.query.get(user.cargo_gema_id)
            cargo_gemas = cargo_gema.nome if cargo_gema else None
            
        return render_template('dashboard.html', user=user, cargo_gemas=cargo_gemas)
    except Exception as e:
...
def admin_edit_user(user_id):
    ...
    if not is_dev_or_admin(current_user):
        log_action('warning', 'Acesso negado', 'Tentativa de editar usuário sem permissão', current_user)
    ...
    # ADMIN só não pode editar DEV
        ...
        log_action('warning', 'Acesso negado', f'Admin tentou editar usuário DEV: {target_user.username}', current_user)
        flash('Você não tem permissão para editar este usuário!', 'error')
        return redirect(url_for('admin_users'))
    
    old_cargo = target_user.cargo.nome if target_user.cargo else 'Nenhum'
    changes = []
    ...
    if cargo_id:
        ...
        if new_cargo and can_manage_cargo(current_user, new_cargo.nivel):
            if target_user.cargo_id != int(cargo_id):
                changes.append(f'Cargo alterado de {old_cargo} para {new_cargo.nome}')
            target_user.cargo_id = cargo_id
    
    # Atualiza cargo de confiança
    old_cargos_confianca = [c.nome for c in target_user.cargos_confianca]
    cargo_confianca_id = request.form.get('cargo_confianca')
    if cargo_confianca_id:
        target_user.cargo_confianca_id = cargo_confianca_id
        new_cargo = CargoConfianca.query.get(cargo_confianca_id)
        if new_cargo:
            changes.append(f'Cargo de confiança alterado para {new_cargo.nome}')
    
    # Atualiza áreas
    old_areas = [a.nome for a in target_user.areas]
    area_ids = request.form.getlist('areas')
    if area_ids:
        new_areas = Area.query.filter(Area.id.in_(area_ids)).all()
        target_user.areas = new_areas
        changes.append(f'Áreas atualizadas: {", ".join([a.nome for a in new_areas])}')
    ...
    if old_status != target_user.is_active:
        changes.append(f'Status alterado para {"ativo" if target_user.is_active else "inativo"}')
    
    try:
        db.session.commit()
        log_action('info', 'Usuário editado', 
                  f'Usuário {target_user.username} editado por {current_user.username}. Alterações: {", ".join(changes)}',
                  current_user)
        flash('Usuário atualizado com sucesso!', 'success')
    except Exception as e:
        db.session.rollback()
        log_action('error', 'Erro ao editar usuário',
                  f'Erro ao editar {target_user.username}: {str(e)}',
                  current_user)
...
def logout():
    if 'username' in session:
        user = User.query.filter_by(username=session['username']).first()
        if user:
            log_action('info', 'Logout', f'Usuário {user.username} fez logout do sistema', user)
    session.clear()
    return redirect(url_for('login'))
...
def delete_cargo_gema(cargo_id):
    ...
    try:
        # Busca usuários afetados antes de excluir o cargo
        users = User.query.filter_by(cargo_gema_id=cargo_id).all()
        
        # Registra a exclusão no log
        log_action('warning', 'Cargo por gemas excluído',
                  f'Cargo {cargo.nome} foi excluído por {current_user.username}.\n' +
                  f'Configuração: Min Gemas: {cargo.min_gemas}, Max Gemas: {cargo.max_gemas}',
                  current_user)
        ...
        # Atualiza os cargos dos usuários afetados
        for user in users:
            cargo_anterior = cargo.nome
            cargo_atual = user.atualizar_cargo_por_gemas()
            cargo_atual_nome = cargo_atual.nome if cargo_atual else "Sem cargo"
            
            log_action('info', 'Cargo por gemas atualizado automaticamente',
                      f'Usuário {user.username} teve seu cargo alterado de {cargo_anterior} para {cargo_atual_nome} ' +
                      f'devido à exclusão do cargo por gemas', current_user)
...
def edit_cargo_gema(cargo_id):
    ...
    try:
        # Guarda os valores antigos para o log
        valores_antigos = {
            'nome': cargo.nome,
        ...
        log_action('info', 'Cargo por gemas editado',
                  f'Cargo {valores_antigos["nome"]} foi editado:\n' +
                  f'Nome: {valores_antigos["nome"]} -> {nome}\n' +
                  f'Min Gemas: {valores_antigos["min_gemas"]} -> {min_gemas}\n' +
                  f'Max Gemas: {valores_antigos["max_gemas"]} -> {max_gemas}',
                  current_user)
        ...
        for user in users:
            cargo_anterior = user.cargo_gema.nome if user.cargo_gema else "Sem cargo"
            cargo_atual = user.atualizar_cargo_por_gemas()
            cargo_atual_nome = cargo_atual.nome if cargo_atual else "Sem cargo"
            
            if cargo_anterior != cargo_atual_nome:
                log_action('info', 'Cargo por gemas atualizado automaticamente',
                          f'Usuário {user.username} teve seu cargo alterado de {cargo_anterior} para {cargo_atual_nome} ' +
                          f'devido à edição do cargo por gemas', current_user)
...
def add_user_gemas(user_id):
    ...
    try:
        ...
        # Atualiza o cargo por gemas
        cargo_anterior = user.cargo_gema.nome if user.cargo_gema else "Sem cargo"
        cargo_atual = user.atualizar_cargo_por_gemas()
        cargo_atual_nome = cargo_atual.nome if cargo_atual else "Sem cargo"
        
        # Registrar a ação no log
        log_action('info', 'Gemas adicionadas', 
                  f'{current_user.username} adicionou {quantidade} gemas para {user.username} (ID: {user.id}). ' + 
                  f'Total atual: {user.total_gemas}. Cargo alterado de {cargo_anterior} para {cargo_atual_nome}', current_user)
...
def remove_user_gemas(user_id):
    ...
    try:
        ...
        if user.total_gemas >= quantidade:
            ...
            # Registrar a ação no log
            log_action('warning', 'Gemas removidas', 
                      f'{current_user.username} removeu {quantidade} gemas de {user.username} (ID: {user.id}). ' + 
                      f'Total atual: {user.total_gemas}. Cargo alterado de {cargo_anterior} para {cargo_atual_nome}', current_user)
...
def admin_update_user(user_id):
    ...
    try:
        ...
        # Registrar mudanças em dados pessoais
        if username != old_username:
            changes.append(f'Username alterado de "{old_username}" para "{username}"')
        if email != old_email:
            changes.append(f'Email alterado de "{old_email}" para "{email}"')
        if discord != old_discord:
            changes.append(f'Discord alterado de "{old_discord}" para "{discord}"')
        if password:
            changes.append('Senha alterada')
        ...
        if email != user.email and User.query.filter_by(email=email).first():
            return jsonify({'success': False, 'message': 'Este email já está em uso'})
        if discord != user.discord and User.query.filter_by(discord=discord).first():
            return jsonify({'success': False, 'message': 'Este ID do Discord já está em uso'})

        # Aplicar alterações em dados pessoais
        ...
        # Atualizar cargo hierárquico
        old_cargo = user.cargo.nome if user.cargo else "Nenhum"
        cargo_id = request.form.get('cargo_id', type=int)
        if cargo_id:
            cargo = Cargo.query.get(cargo_id)
            if cargo:
                user.cargo_id = cargo_id
                changes.append(f'Cargo hierárquico alterado de "{old_cargo}" para "{cargo.nome}"')

        # Atualizar cargos de confiança
        old_cargos = [c.nome for c in user.cargos_confianca]
        cargos_confianca_ids = request.form.getlist('cargos_confianca')
        if cargos_confianca_ids:
            new_cargos = CargoConfianca.query.filter(
                CargoConfianca.id.in_(cargos_confianca_ids),
                CargoConfianca.is_active == True
            ).all()
            user.cargos_confianca = new_cargos
            new_cargos_nomes = [c.nome for c in new_cargos]
            changes.append(f'Cargos de confiança alterados de "{", ".join(old_cargos)}" para "{", ".join(new_cargos_nomes)}"')

        # Atualizar áreas
        old_areas = [a.nome for a in user.areas]
        area_ids = request.form.getlist('areas')
        if area_ids:
            new_areas = Area.query.filter(Area.id.in_(area_ids)).all()
            user.areas = new_areas
            new_areas_nomes = [a.nome for a in new_areas]
            changes.append(f'Áreas alteradas de "{", ".join(old_areas)}" para "{", ".join(new_areas_nomes)}"')

        # Atualizar status
        old_status = "ativo" if user.is_active else "inativo"
        new_status = 'is_active' in request.form
        if old_status != new_status:
            user.is_active = new_status
            changes.append(f'Status alterado de "{old_status}" para "{"ativo" if new_status else "inativo"}"')

        # Registrar todas as alterações no log
        if changes:
            log_action('info', 'Usuário atualizado',
                      f'{current_user.username} atualizou o usuário {user.username} (ID: {user.id}). ' +
                      f'Alterações: {"; ".join(changes)}', current_user)
...
def admin_logs():
    ...
    if date:
        ...
        query = query.filter(Log.timestamp >= date_obj, Log.timestamp < next_day)
    if search:
        search_term = f"%{search}%"
        if filter_type == 'username':
            query = query.filter(db.or_(
                Log.details.ilike(f"%Username alterado%{search}%"),
                Log.details.ilike(f"%usuário {search}%"),
                Log.username.ilike(search_term)
            ))
        elif filter_type == 'email':
            query = query.filter(Log.details.ilike(f"%Email alterado%{search}%"))
        elif filter_type == 'discord':
            query = query.filter(Log.details.ilike(f"%Discord alterado%{search}%"))
        elif filter_type == 'cargo':
            query = query.filter(db.or_(
                Log.details.ilike(f"%Cargo hierárquico alterado%{search}%"),
                Log.details.ilike(f"%Cargos de confiança alterados%{search}%")
            ))
        elif filter_type == 'area':
            query = query.filter(Log.details.ilike(f"%Áreas alteradas%{search}%"))
        elif filter_type == 'gemas':
            query = query.filter(db.or_(
                Log.action == 'Gemas adicionadas',
                Log.action == 'Gemas removidas'
            ))
            if search:
                query = query.filter(Log.details.ilike(search_term))
        else:
            query = query.filter(db.or_(
                    Log.action.ilike(search_term),
                    Log.details.ilike(search_term),
                    Log.username.ilike(search_term)
            ))
...
def admin_clear_logs():
    current_user = get_current_user()
    if not is_dev_or_admin(current_user) or current_user.username != 'allef':
        log_action('warning', 'Acesso negado', 'Tentativa não autorizada de limpar logs', current_user)
        flash('Apenas o usuário allef pode limpar os logs.', 'error')
        return redirect(url_for('admin_logs'))
    
    try:
        # Registrar a ação de limpeza antes de deletar os logs
        log_action('warning', 'Logs limpos', 'Todos os logs foram removidos', current_user)
        
        # Deletar todos os logs exceto o último (que é o registro da limpeza)
        Log.query.filter(Log.id != db.session.query(db.func.max(Log.id))).delete()
        db.session.commit()
